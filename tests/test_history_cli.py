        ...
        mock_get_history.return_value = [
            {
                "text": "Quote 1",
                "author": "Author 1",
                "shown_at": "2023-01-01 12:00:00",
            },
            {
                "text": "Quote 2",
                "author": "Author 2",
                "shown_at": "2023-01-01 12:01:00",
            },
        ]

        result = runner.invoke(
            app, ["history", "list", "--working-dir", str(working_dir)]
        )
...
def test_history_clear_command(tmp_path):
    ...
    with (
        ...
        result = runner.invoke(
            app, ["history", "clear", "--force", "--working-dir", str(working_dir)]
        )
        assert result.exit_code == 0
...
def test_history_stat_command(tmp_path):
    ...
    with (
        ...
        patch("zen_prompt.commands.history.sqlite3.connect") as mock_connect,
        patch(
            "zen_prompt.commands.history.db.get_history_stats"
        ) as mock_get_history_stats,
    ):
        mock_get_db.return_value = str(db_path)
        mock_conn = MagicMock()
        mock_connect.return_value = mock_conn
        ...
        result = runner.invoke(
            app, ["history", "stat", "--working-dir", str(working_dir)]
        )
        assert result.exit_code == 0
